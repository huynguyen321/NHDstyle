...
var listLocal = function() {
    ...
    for (var i in product) {
        ...
        listproduct = '<div class="col-lg-2 col-md-2 col-2 mt-2">';
        listproduct += '<div class="card product p-2" style="width: 170px; height: 405px; background-color: #ADA5A5">';
        ...
        listproduct += '<br>';
        listproduct += '<div class="price text-center h6" style="color: blue">' + data.price + 'VND</div>';
        listproduct += '<div class="star_box float-left pt-2">';

        listproduct += '<span style="background-color: red; border: red;" class="text-center add-to-cart btn btn-outline-warning add-cart float-left col-4" data-id="' + data.id + '" data-name="' + data.name + '" data-img="' + data.img + '" data-price="' + data.price + '" onclick="tks()">';
        listproduct += '<a>';
        ...
        listproduct += '<button style="background-color: blue"class="btn btn-primary float-right" data-toggle="modal" data-target="#exampleModalLong" onclick="getProductById(' + i + ')">';
        listproduct += 'Chi tiết';
        listproduct += '</button>';
